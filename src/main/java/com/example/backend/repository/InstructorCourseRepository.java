...
public interface InstructorCourseRepository extends JpaRepository<InstructorCourse, Integer> {
    ...
    @Query(value = "select * from course_instructor where course_id = ?  and deleted = 0",
            nativeQuery = true)
    List<InstructorCourse> getInstructorCourseByIdCourse(int id_course);

    @Query(value = "select * from course_instructor where deleted = 0", nativeQuery = true)
    List<InstructorCourse> getListCourse();
